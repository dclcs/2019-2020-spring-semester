    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Schlick_F(half3 R, half cosA)
            {
                ...
                half3 specColor;
                float3 albedo = DiffuseAndSpecularFromMetallic (R, cosA, /*out*/ specColor, /*out*/ oneMinusReflectivity);

                return specColor + (1 - specColor) * oneMinusReflectivity;
            }

            float GGX_D(float roughness, float NdotH)
            {
                return pow(roughness, 2.0) / (UNITY_PI * pow( pow(NdotH, 2.0) * (pow(roughness, 2.0) - 1) + 1, 2)); 
            }
