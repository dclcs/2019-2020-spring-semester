...
public class PendulumTypeDController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float middleTheta = (theta + lastTheta) / 2;
        float middleOmega = (omega + lastOmega) / 2;
        gameObject.transform.position += (float)System.Math.Cos(middleTheta) * middleOmega * distance * targetVector * Time.deltaTime;
        gameObject.transform.position += (float)System.Math.Sin(middleTheta) * middleOmega * distance * upVector * Time.deltaTime;
